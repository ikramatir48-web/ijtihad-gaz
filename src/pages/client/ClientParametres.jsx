...
export default function ClientParametres() {
  ...
  async function savePassword() {
    ...
    try {
      ...
      await supabase
        .from('profiles')
        .update({ force_password_change: false, password_changed: true })
        .eq('id', profile.id);
      ...
      <div className="flex gap-2 mb-4">
        <button
          ...
          className={`btn btn-sm ${
            tab === 'profil' ? 'btn-primary' : 'btn-ghost'
          }`}
        >
        ...
        <button
          ...
          className={`btn btn-sm ${
            tab === 'securite' ? 'btn-primary' : 'btn-ghost'
          }`}
        >
      ...
      {tab === 'securite' && (
        <div className="card" style={{ maxWidth: 520 }}>
          ...
          <div className="form-group">
            ...
            <div style={{ position: 'relative' }}>
              <input
                ...
                value={pwd.ancien}
                onChange={(e) =>
                  setPwd((p) => ({ ...p, ancien: e.target.value }))
                }
            ...
            {pwd.nouveau && (
              <div
                style={{
                  marginTop: 6,
                  fontSize: 12,
                  color: pwdStrength ? 'var(--danger)' : 'var(--success)',
                }}
              >
          ...
          <button
            ...
            disabled={saving || !!pwdStrength || !pwd.ancien}
          >
            {saving ? <span className="spinner" /> : <Lock size={14} />} Mettre
            à jour
          </button>
        </div>
      )}
