...
export default function ClientNouvelleCommande() {
  ...
  const lignesPanier = produits
    .filter((p) => panier[p.id] > 0)
    .map((p) => ({
      ...p,
      quantite: panier[p.id],
      subtotal: p.prix_unitaire * panier[p.id],
    }));
  ...
  async function passerCommande() {
    ...
    try {
      ...
      await supabase
        .from('profiles')
        .update({ mode_reglement: modeReglement })
        .eq('id', profile.id);
      ...
      if (dateLivraison) {
        ...
        for (const admin of admins || []) {
          await supabase.from('notifications').insert({
            user_id: admin.id,
            type: 'info',
            titre: `Livraison souhaitée le ${new Date(
              dateLivraison
            ).toLocaleDateString('fr-FR')}`,
            message: `${profile.nom} souhaite être livré le ${new Date(
              dateLivraison
            ).toLocaleDateString('fr-FR', {
              weekday: 'long',
              day: 'numeric',
    ...
    return (
      ...
      >
        ...
        >
          ...
          <div className="badge badge-orange" style={{ margin: '0 auto 16px' }}>
            {cmdNumero}
          </div>
          <p
          ...
          >
            Votre commande a bien été enregistrée. Le bon de livraison sera
            disponible dès confirmation.
          </p>
          ...
          <div key={n} className="flex items-center gap-3">
            <div
              style={{
                ...
                justifyContent: 'center',
                background:
                  step >= Number(n) ? 'var(--accent)' : 'var(--bg-elevated)',
                color: step >= Number(n) ? 'white' : 'var(--text-muted)',
                fontWeight: 700,
                fontSize: 13,
              }}
            >
            ...
            <span
              style={{
                fontSize: 13.5,
                fontWeight: 500,
                color: step >= Number(n) ? 'var(--text)' : 'var(--text-muted)',
              }}
            >
              {label}
            </span>
            ...
            <div className="card">
                ...
                {produits.map((p) => (
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: 12,
                      border: `2px solid ${
                        panier[p.id] ? 'var(--accent)' : 'transparent'
                      }`,
              ...
              <div className="form-group">
                ...
                <textarea
                  ...
                  placeholder="Entrez vos instructions de livraison..."
                />
              </div>
            </div>
          ) : (
            <div className="card">
              ...
              >
                ...
                <div className="text-muted text-sm">
                  {profile?.numero_client} — {profile?.email}
                </div>
              </div>
              ...
              {lignesPanier.map((l, i) => (
                ...
                >
                  <div>
                    ...
                    <div className="text-muted text-sm">
                      {Number(l.prix_unitaire).toFixed(2)} DH × {l.quantite}
                    </div>
                  </div>
                  <div style={{ fontWeight: 700 }}>
                    {l.subtotal.toFixed(0)} DH
                  </div>
                </div>
              ))}
              ...
              >
                ...
                <strong style={{ color: 'var(--text)' }}>
                  {modeReglement === 'esp' ? 'Espèces' : 'Chèque'}
                </strong>
              </div>
