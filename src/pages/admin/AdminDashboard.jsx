...
export default function AdminDashboard() {
  ...
  async function activerCompte(clientId, nom) {
    ...
    await supabase.from('notifications').insert({
      ...
      titre: 'Compte activé',
      message:
        'Bienvenue parmi nos fidèles clients. Vous pouvez désormais passer commande.',
    });
  ...
  async function load() {
    const [cmdRes, clientRes, livrRes] = await Promise.all([
      supabase
        .from('commandes')
        .select(
          'id, statut, created_at, numero_commande, mode_paiement, date_livraison_souhaitee, profiles(nom, numero_client)'
        )
        .order('created_at', { ascending: false })
        .limit(8),
      supabase
      ...
      supabase
        .from('commandes')
        .select(
          'id, numero_commande, date_livraison_souhaitee, statut, profiles(nom, telephone)'
        )
  ...
  return (
    <div>
        ...
        <div className="modal-overlay">
          <div className="modal" style={{ maxWidth: 480 }}>
            ...
            <div className="modal-body">
              ...
              >
                {pendingClients.length} nouveau(x) client(s) attendent la
                validation de leur compte. Appelez-les pour confirmer leur
                adresse avant d'activer.
              </p>
              {pendingClients.map((c) => (
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() => activerCompte(c.id, c.nom)}
                  >
      ...
      <div
        style={{
          display: 'grid',
          gridTemplateColumns: livraisons.length > 0 ? '1fr 340px' : '1fr',
          gap: 20,
        }}
      >
        {/* Commandes récentes */}
                  ...
                  {recent.map((cmd) => (
                    ...
                    >
                      ...
                      <td>
                        ...
                        <div className="text-muted text-sm">
                          {cmd.profiles?.numero_client}
                        </div>
                      </td>
                      <td className="text-muted">
                        {format(new Date(cmd.created_at), 'dd MMM yyyy', {
                          locale: fr,
                        })}
                      </td>
                      <td>
        ...
        {livraisons.length > 0 && (
          <div className="card">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {livraisons.map((cmd) => {
                const { label, color } = getLivraisonLabel(
